(ns plastic.frame.middleware
  (:require
    [reagent.ratom :refer [IReactiveAtom]]
    [plastic.frame.undo :refer [store-now!]]
...
(defn pure
  ...
  is to use the low level registration function \"plastic.frame.handlers/register-handler-base\""
  [handler]
  (fn pure-handler
    [app-db event-vec]
