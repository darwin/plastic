(ns plastic.main.frame.undo
  ...
  (:require
    ...
    [plastic.main.frame.handlers :as handlers]
...
;;
;;
(defonce ^:private max-undos "Maximum number of undo states maintained" (atom 50))
(defn set-max-undos!
  [n]
  (reset! max-undos n))


(defonce ^:private undo-list "A list of history states" (reagent/atom []))
(defonce ^:private redo-list "A list of future states, caused by undoing" (reagent/atom []))
...
(defonce ^:private app-explain "Mirrors app-db" (reagent/atom ""))
(defonce ^:private undo-explain-list "Mirrors undo-list" (reagent/atom []))
(defonce ^:private redo-explain-list "Mirrors redo-list" (reagent/atom []))
