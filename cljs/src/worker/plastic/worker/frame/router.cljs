(ns plastic.worker.frame.router
  ...
  (:require [reagent.core :refer [flush]]
            [plastic.worker.frame.handlers :refer [handle]]
...
;;
(defonce ^:private event-chan (chan))                       ;; TODO: set buffer size?
      ...
      (try
        ...
        (catch js/Object e
          (do
            ...
            (router-loop db)                                   ;; Exception throw will cause termination of go-loop. So, start another.
