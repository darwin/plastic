(ns plastic.worker.frame.middleware
  (:require
    [reagent.ratom :refer [IReactiveAtom]]
    [plastic.worker.frame.undo :refer [store-now!]]
...
(defn pure
  ...
  is to use the low level registration function \"plastic.main.frame.handlers/register-handler-base\""
  [handler]
  (fn pure-handler
    [app-db event-vec]
